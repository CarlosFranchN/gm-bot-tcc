...
def executar_tribunal_completo(provedor: str = "groq"):
    ...
    for arquivo_path in transcripts:
        ...
        for turno in turnos_jogo:
            ...
            try:
                # O Juiz (judge_creative.py) já tem os retries e os time.sleeps longos embutidos!
                ...
                # Se o juiz devolver None, é porque todas as tentativas esgotaram.
                if notas_brutas is None:
                    print(f"  ⚠️ Turno {turno['turno']} falhou após todas as tentativas. Posição ignorada nas métricas.")
                    continue 
                ...
                # 👉 Respiro de prevenção: Evita bater no limite da Groq. 
                # 5 a 10 segundos costuma ser ideal para o limite de 30/minuto deles.
                time.sleep(10) 
                
            except Exception as e:
                # Este except agora captura apenas erros locais (ex: o JSON do transcript estava corrompido, 
                # ou a função normalizar_notas quebrou), não mais os erros da API.
